...
function ProductDetails({ addToCart }) {
  ...
  useEffect(() => {
    async function fetchProduct() {
      try {
        const response = await axios.get(`https://api-ecom.thapatechnical.shop/api/products/${id}`);
        if (response.data) {
  ...
  const handleRatingChange = async (event, newRating) => {
    ...
    try {
      await axios.put(`https://api-ecom.thapatechnical.shop/api/products/${id}/rating`, { rating: newRating });
